"""Static checks for generated notebooks.

These checks are intentionally light enough for GitHub Actions. They verify the
published notebooks are clean, parseable, and do not contain obvious secrets.
The expensive GPU transcription path should be tested manually or in a separate
...
def check_notebook(path: Path) -> list[str]:
    ...
    for index, cell in enumerate(nb.get("cells", []), start=1):
        ...
        if "@@P" in text:
            errors.append(f"{path}: cell {index} contains an unresolved protected token")
...
def main() -> int:
    ...
    for path in sorted(LOCALES_DIR.glob("*.json")):
        try:
            ...
            continue
        if "???" in text:
            errors.append(f"{path}: contains likely mojibake marker '???'")
        if "@@P" in text:
            errors.append(f"{path}: contains an unresolved protected token")
